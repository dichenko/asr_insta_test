...
<p>Если пользователь подключает Facebook для анализа конкурентов, сервис может сохранить Facebook user id, список доступных Facebook Pages, связанную Instagram Business/Creator учетную запись и зашифрованные access tokens, необходимые для запросов к Meta API.</p>
<p>Если пользователь отправляет ссылку на аккаунт конкурента, сервис может получить и сохранить открытые данные этого Instagram-аккаунта, доступные через официальный API Instagram: username, описание профиля, публичные счетчики и последние публикации.</p>
<p>Данные используются только для подготовки аналитического отчета и проверки MVP-сценария.</p>
<p>Access tokens Instagram и Facebook/Page хранятся в зашифрованном виде. Секреты и токены не публикуются и не передаются третьим лицам, кроме API Instagram/Meta и OpenAI для выполнения анализа.</p>
...
<p>Анализ конкурентов строится только на открытых данных, доступных через Meta API, и не содержит приватных Insights чужих аккаунтов.</p>
""",
    )
