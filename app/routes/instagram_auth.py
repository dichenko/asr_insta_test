...
async def start_instagram_oauth(state: str = Query(...), session: AsyncSession = Depends(get_session)) -> RedirectResponse | HTMLResponse:
...
async def instagram_callback(
    ...
    if auth_session is None:
        return _html("Invalid link", "<p>This connection link is invalid. Please return to Telegram and request a new link.</p>")
    if await get_valid_auth_session(session, state) is None:
        return _html("Link expired", "<p>This connection link has expired. Please return to Telegram and request a new link.</p>")
    ...
    try:
        ...
        return _html("Token exchange failed", "<p>Could not connect Instagram. Please return to Telegram and request a new link.</p>")

    access_token = short_token_data.get("access_token")
    if not access_token:
        auth_session.status = "failed"
        await session.commit()
        return _html("Token exchange failed", "<p>Instagram did not return an access token.</p>")
